...
const MAX_RECOIL = Math.max(0,
  ...
  // this is a different animation, and only one plays at a time.
  ...monsterAttack.frames.map(f => f.hero));
...
const MAX_MONSTER_BACK = Math.max(0,
  ...monsterAttack.frames.map(f => -f.shove),
...
// The monster's displacement and mark for `frame` of its own attack.
function monsterAttackFrame(frame) {
  const f = monsterAttack.frames[Math.min(Math.max(0, frame), monsterAttack.frames.length - 1)];
