...
//
// Every script above is six frames, and both blow anims last 1500ms, so for a
// long time each frame was a flat 1500/6 = 250ms. That grid was measured on
// 2026-07-31 against the thing it actually has to survive — how often the
// statusline is redrawn — and it was spending its budget in the wrong place.
//
...
//
// Which is what made the flat grid expensive. Frames 0 and 5 of every script are
// the at-rest frame — `pose: null`, `back: 0`, `fly: null` — because a script
// has to open and close on the hero's mark (see `attacks`). On a flat grid those
// two frames are 500ms of a 1500ms blow, so **a third of the samples land on art
// that is identical to standing still**. With one or two samples per blow, a
// third of the time the animation genuinely does not render.
//
// Worse, the miss is biased rather than random. Redraws are not only on the 1s
// tick: a tool finishing triggers one, and that is the same event that makes the
// hook fold and queue the blow. Those event-driven redraws were logged at 155,
// 174, 205, 212, 289, 328 and 385ms after the anim's own timestamp — every one
// of them inside a 250ms opening frame. The redraw *caused by* the blow was the
// one guaranteed to draw the hero standing still.
//
// So the frames are weighted instead. The rest frames keep their place — the
// invariant is that the script opens and closes on the mark, not that it lingers
// there — but they are cut to the minimum that still reads as a boundary, and
// the four posed frames take the 350ms they release. Nothing else moves: still
// six frames, still 1500ms, still `hitFrame: 3`, which now begins at 800ms
// against the old 750. Dead art falls from 500ms to 150ms, and every one of the
// event-driven redraws above now lands on the wind-up.
const BLOW_MS = [100, 350, 350, 350, 300, 50];

// Weights are per *position* rather than per script, because all five scripts —
// the four classes and the monster's — are the same six frames with the same
