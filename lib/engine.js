...
function newState(clsId, name, now) {
  const state = {
    ...
    counters: {
      ...
      linesWritten: 0, goldEarned: 0, deaths: 0, lastTestXpAt: 0, insightEarned: 0,
    },
...
function rollLoot(state, mLvl, opts, rand, now) {
  ...
  const floor = opts.floorEpic ? B.GOBLIN_RARITY_FLOOR : (opts.floorRare ? B.BOSS_RARITY_FLOOR : -1);
  if (floor >= 0) {
...
function goblinFlees(state, rand, now) {
  const m = state.monster;
...
module.exports = {
  ...
  addXp, dealDamage, hurtHero, retaliate, rollLoot, addToInventory, worthKeeping, enqueue, tick,
