...
const BLOWS = new Set(['hit', 'mhit']);

// The same two are also the whole of the *blow lane*, which is the other thing
// this set decides. Everything queued here used to share one timeline: `at` was
// measured off the last anim of any kind, so a blow that arrived during a kill
// (2500ms) and the level-up behind it (5000ms) was scheduled after both. Measured
// in play, a hit sat queued 6.5 seconds — and while it waited every further hit
// coalesced into it, so the hero visibly never swung and then swung once, long
// after the work that earned it.
//
// A blow and a banner are not competing for the same thing, though: a blow is
// what the sprites are doing and a banner is what the text line says, and the
// renderer draws both at once (see `blow`/`banner` in the statusline). So they
// get a lane each. Within a lane the old serialisation is unchanged — two blows
// still queue, because they are the same pair of sprites — and across lanes
// nothing waits for anything.
function laneOf(type) { return BLOWS.has(type) ? 'blow' : 'banner'; }

// The banners that are not only a line of text: each of these puts something on
// the sprites too — a corpse, an empty field where a monster fled, a hero who
// just died — and what they put there contradicts a blow. So they are the one
// place the lanes still touch, in both directions and for the same reason:
//
//   * scheduling — a scene banner starts no earlier than the blow lane is clear,
//     because the killing blow is *why* there is a corpse to draw. Enqueued flat,
//     the `kill` would start on the same millisecond as the hit that earned it
//     and the game's most satisfying frame would never be drawn at all.
//   * drawing — while one is on screen the renderer suppresses any blow that
//     turns up underneath it (see SCENE_BANNERS in the statusline, which reads
//     this set), rather than queueing it. A blow is cosmetic; its damage was
//     banked when the event was folded.
//
// Every other banner leaves both combatants where they were, so it neither waits
// for a blow nor hides one.
const SCENE_BANNERS = new Set(['kill', 'bossdown', 'cleared', 'death', 'travel', 'goblinflee']);

function enqueue(state, type, dur, data, now) {
  const q = state.anim;
  const lane = laneOf(type);
  const holdsFor = SCENE_BANNERS.has(type) ? null : lane;   // null: wait on both
  let last = null;
  for (let i = q.length - 1; i >= 0; i--) {
    if (holdsFor && laneOf(q[i].type) !== holdsFor) continue;
    if (!last || q[i].at + q[i].dur > last.at + last.dur) last = q[i];
  }
  if (BLOWS.has(type) && last && last.type === type && last.at + last.dur > now) {
  ...
  // Counted per lane rather than over the queue, for the same reason the clock
  // is: a cap the two lanes share is a way for a run of banners to keep blows
  // off the screen, which is the thing the lanes exist to stop. The queue can
  // hold twice ANIM_CAP now, and every entry is four fields.
  if (q.reduce((n, a) => n + (laneOf(a.type) === lane ? 1 : 0), 0) >= B.ANIM_CAP) return;
  q.push({ type, at, dur, data: data || {} });
}
// The live `hit` that a counter, a near miss or a corpse tag belongs to.
//
// All three used to read `state.anim[state.anim.length - 1]`, which was the same
// thing back when one timeline meant a hit was always the last entry pushed. It
// is not any more: `enqueue` pushes in call order, a blow can coalesce into a
// hit that is already in the queue, and a banner queued in between then sits on
// the end of the array with that hit still live behind it. Reading the end of
// the array dropped the `↩-N` on exactly those exchanges — and dropped it
// silently, because `hurtHero` had already taken the HP off.
function liveHit(state, now) {
  let best = null;
  for (const a of state.anim) {
    if (a.type !== 'hit' || a.at + a.dur <= now) continue;
    if (!best || a.at + a.dur > best.at + best.dur) best = a;
  }
  return best;
}
...
function resolveKill(state, opts, rand, now) {
  ...
  const blow = liveHit(state, now);
  if (blow) blow.data.mon = corpse;
...
function retaliate(state, rand, now) {
  ...
  const roll = rand();
  if (roll >= chance) {
    if (roll < chance + B.NEAR_MISS_BAND) markDodge(state, now);
    return;
  }
...
// Tag the hit anim the near miss belongs to, if there is still one on screen.
// Deliberately silent when a counter has already landed on that record — see the
// note above.
function markDodge(state, now) {
  const last = liveHit(state, now);
  if (last && !last.data.counter) last.data.dodged = true;
}
...
module.exports = {
  ...
  addXp, dealDamage, resolveKill, hurtHero, monsterStrikes, retaliate, rollLoot, addToInventory, worthKeeping, enqueue, tick, laneOf, SCENE_BANNERS,
