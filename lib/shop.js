...
//
// `classId` only picks which noun list each offer is named from — it is
// deliberately *not* in the seed, so a Ranger and a Wizard standing in the same
// grove in the same window see the same five offers at the same ilvls, rarities
// and prices, one shelf of bows and one of wands. `pick` burns exactly one
// rand() call whatever the list length, so the stream downstream of the noun is
// identical too.
function rollStock(zoneId, now, classId) {
  ...
  const rolls = slots.map(st => ({
    ...
    noun: pick(C.nounsFor(st.id, classId), rand),
...
// What an offer is worth *to this hero*, judged after the roll rather than
// during it. The roll stays honest — `rollStock` never sees the hero's gear, so
// a Ranger and a Wizard in the same grove still read the same five offers, and
// the shelf keeps its "hunt the one good slot" shape instead of degrading into
// a vending machine that always has your upgrade in stock.
//
// What changes is that a dead offer now *says* it is dead. Rolling ilvl
// uniformly over the zone span means slot 1 can be a byte-identical copy of the
// weapon you are wearing at full price, and slot 5 can be an i1 the same hero
// out-classes twelvefold — which is fine as a roll and a trap as a listing,
// because nobody diffs twelve worn slots by eye before spending 840g.
//
// `verdict` is one of:
//   'upgrade' — fits an empty slot, or beats the weakest thing worn in its slot
//   'same'    — ranks exactly level with it; you are already wearing this
//   'worse'   — strictly beaten, and no amount of gold changes that
function offerVerdict(state, offer) {
  if (E.worthKeeping(state, offer)) return 'upgrade';
  const keys = C.slotKeys(offer.slot);
  let weakest = Infinity;
  for (const key of keys) {
    const worn = state.equipment[key];
    if (worn) weakest = Math.min(weakest, E.itemValue(worn));
  }
  return E.itemValue(offer) === weakest ? 'same' : 'worse';
}

// Tag every offer on a shelf with its verdict, and take the SALE flag off the
// ones that can't be worn. A discount is a reason to look twice, so hanging one
// on a strictly-dominated item aims the player's eye at the worst line on the
// shelf — the single most expensive mistake this listing can cause. The price
// stays discounted; only the flourish that *advertises* it goes.
function appraise(state, offers) {
  return offers.map(o => {
    const verdict = offerVerdict(state, o);
    return { ...o, verdict, sale: o.sale && verdict === 'upgrade' };
  });
}
...
function refresh(state, now) {
  ...
  state.shop = rollStock(zoneId, now, state.hero.class);
  return {
    stock: state.shop,
