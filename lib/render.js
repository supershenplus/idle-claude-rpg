...
// The three HUD layouts and the terminal width each one needs. These used to be
...
const HUD_MODES = ['big', 'compact', 'mini'];
const HUD_MIN_COLS = { big: 76, compact: 50, mini: 0 };

// The layout a given width picks on its own, absent any preference.
function hudFor(cols) {
  return cols >= HUD_MIN_COLS.big ? 'big' : cols >= HUD_MIN_COLS.compact ? 'compact' : 'mini';
}
