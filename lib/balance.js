...
// How much of the roll *above* the retaliation threshold is narrated as a swing
// you dodged rather than as no swing at all. Purely a reading of the same roll:
// the monster's expected damage is untouched, and `retaliate` still spends
// exactly one random number whatever it decides, so no tuned outcome and no
// seeded test moves because of it.
//
// The band is what keeps the animation worth watching. Every non-counter could
// be called a dodge — that is around 70% of hits, at which point the hero is
// perpetually mid-lean and the tell means nothing. A fifth of hits is often
// enough to be seen within a session and rare enough to still read as an event,
// and the rest stay silent, which is also honest: most of the time nothing
// happened. Applied to bosses and goblins as a band above *their* thresholds,
// so a boss dodges as often as trash does even though it swings more.
const NEAR_MISS_BAND = 0.20;
...
// The loot goblin — a sub-boss that turns up in place of trash and pays out
// once, in one of two ways. Every number here is a *multiple of what the same
// spawn would have been*, never an absolute, which is the whole safety property:
// a goblin met at level 8 in the Grove pays Grove rates, so it stays a windfall
// instead of handing a level-8 hero Null Expanse gear.
const GOBLIN_CHANCE = 0.05;       // of non-boss spawns
const GOBLIN_HP_MULT = 3;         // tankier than trash, nowhere near the boss's ×10
const GOBLIN_XP_MULT = 3;         // it costs three trash mobs of time; pay for that
const GOBLIN_GOLD_MULT = 8;       // the gold arm, against this monster's own killGold
const GOBLIN_EPIC_CHANCE = 0.3;   // …otherwise a guaranteed epic at its own ilvl
const GOBLIN_RARITY_FLOOR = 3;    // index into RARITIES: epic or better

// It bites. A sack of gold guarded by nothing is a vending machine, not an
// event — and the sim proved that literally: with the goblin as a fat trash mob
// carrying a prize, the attentive hero over-geared and stopped dying (1 death in
// 90 days against a floor of one per 30). Retaliation sits between trash and
// boss, and because the fight lasts ×3 as long, total exposure is roughly nine
// times a trash mob's. The prize is now something you can fail to collect.
const GOBLIN_RETALIATE_CHANCE = 0.40;   // trash 0.30, boss 0.45
const GOBLIN_RETALIATE_MULT = 1.0;      // trash 0.45, boss 1.6

// …and it runs. Counted in folded events, never in seconds: a wall-clock timer
// would be the only thing in this engine whose outcome depends on when you
// happened to look, and `fold` exists precisely so a replay lands where the
// live run did. The goblin gets this many events to survive; kill it inside
// them and the sack is yours.
//
// 10 is measured, not guessed. Escape rates over six seeds × 90 days, by how
// well the hero is kept up:
//
...
//
// So a hero who wears what they find loses about one goblin in seventeen —
// rare enough to sting, common enough that "get it before it runs" is not a
// bluff — while a hero who never opens their inventory loses half of them.
// That spread is the point: the deadline is a DPS check, so gear is the whole
// defence, and falling behind is what makes the goblin start getting away.
const GOBLIN_FLEE_EVENTS = 10;
...
module.exports = {
  ...
  GOBLIN_CHANCE, GOBLIN_HP_MULT, GOBLIN_XP_MULT, GOBLIN_GOLD_MULT,
  GOBLIN_EPIC_CHANCE, GOBLIN_RARITY_FLOOR,
  GOBLIN_RETALIATE_CHANCE, GOBLIN_RETALIATE_MULT, GOBLIN_FLEE_EVENTS,
