...
function snapshotPreMigration(raw, from) {
  ...
  try {
    const dest = path.join(P.CHAR_DIR, `state.v${from}.json`);
...
function quarantineCorrupt() {
  try {
    const dir = P.CHAR_DIR;
    const dest = path.join(dir, `state.corrupt-${Date.now()}.json`);
    fs.renameSync(P.stateFile, dest);
    const corrupts = fs.readdirSync(dir)
      .filter(f => f.startsWith('state.corrupt-')).sort();
    while (corrupts.length > MAX_CORRUPT_FILES) {
      fs.unlinkSync(path.join(dir, corrupts.shift()));
    }
  } catch (_) { /* best effort */ }
}
...
// Everything on disk that belongs to one hero. `delete` and `reset` both
// promise "forever", so the list has to cover the files a save spills into as
// well as the fixed ones — a quarantined corrupt copy and a pre-migration
...
function saveFilesFor(slug) {
  const dir = P.charDir(slug);
  const gens = [];
  for (let n = 1; n <= P.BAK_GENERATIONS; n++) gens.push(P.bakGenFor(slug, n));
  const fixed = [P.stateFileFor(slug), ...gens, P.bakFileFor(slug)];
  let spilled = [];
  try {
    spilled = fs.readdirSync(dir)
      .filter(f => /^state\.(corrupt-\d+|v\d+)\.json$/.test(f))
      .map(f => path.join(dir, f));
  } catch (_) { /* no such character: the fixed names are answer enough */ }
  return [...fixed, ...spilled];
}

function saveFiles() { return saveFilesFor(P.activeSlug()); }

// The files no character owns: the shared inbox, the shared lock, and the
// pointer saying who is playing. Only `reset` touches these — deleting one
// character has no business emptying the inbox the others are still fed from.
function globalFiles() {
  return [P.eventsFile, P.processingFile, P.lockFile, P.activeFile];
}

// One line of the roster. Deliberately a raw read rather than `loadState`: this
// runs once per character on `/hero roster`, and migrating — or worse,
// *recovering* — every hero on the machine to print a list would turn a read
// into eleven writes. A save too old or too broken to read still gets a row,
// because a roster that silently omits a hero is the one thing it must not do.
function describe(slug) {
  const row = { slug, name: null, class: null, level: null, zone: null, playedAt: 0 };
  try { row.playedAt = fs.statSync(P.stateFileFor(slug)).mtimeMs; } catch (_) { /* none */ }
  try {
    const s = JSON.parse(fs.readFileSync(P.stateFileFor(slug), 'utf8'));
    const h = (s && s.hero) || {};
    Object.assign(row, {
      name: typeof h.name === 'string' ? h.name : null,
      class: typeof h.class === 'string' ? h.class : null,
      level: Number.isFinite(h.level) ? h.level : null,
      zone: typeof h.zone === 'string' ? h.zone : null,
      gold: Number.isFinite(h.gold) ? h.gold : 0,
      clearedAt: h.clearedAt || null,
      version: s && s.version,
    });
  } catch (_) { /* unreadable: the row says so by having no name */ }
  return row;
}

function roster() { return P.listSlugs().map(describe); }
...
module.exports = {
  loadState, saveState, hasSave, saveFiles, saveFilesFor, globalFiles,
  describe, roster, appendEvent, tryFold,
